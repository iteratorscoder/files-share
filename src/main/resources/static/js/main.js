...
function uploadSingleFile(file) {
    ...
    xhr.onload = function() {
        ...
        if(xhr.status === 200) {
            ...
            singleFileUploadSuccess.innerHTML = "<p>File Uploaded Successfully.</p><p>DownloadUrl : <a href='" + response.fileDownloadUri + "' target='_blank'>" + response.fileName + "</a></p>";
    ...
    xhr.onload = function() {
        ...
        if(xhr.status === 200) {
            ...
            for(let i = 0; i < response.length; i++) {
                content += "<p>DownloadUrl : <a href='" + response[i].fileDownloadUri + "' target='_blank'>" + response[i].fileName + "</a></p>";
            }
